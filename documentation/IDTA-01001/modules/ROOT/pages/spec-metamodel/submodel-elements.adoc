...
Handling of operations in Submodel with xref:spec-metamodel/common.adoc#HasKind[kind]=xref:spec-metamodel/common.adoc#ModellingKind[Instance] are handled identical to operations in Submodel Templates (Submodel with xref:spec-metamodel/common.adoc#HasKind[kind]=xref:spec-metamodel/common.adoc#ModellingKind[Template])
...
A two-dimensional array list[3\]][5\] with _Property_ values would be realized like follows: the first SubmodelElement list would contain three xref:spec-metamodel/submodel-elements.adoc#submodel-element-list-attributes[SubmodelElementList] elements.
