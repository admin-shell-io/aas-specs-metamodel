...
xref:spec-metamodel/core.adoc#submodel-attributes[Submodel]s consist of a set of SubmodelElements.
SubmodelElements may be qualified by a so-called xref:spec-metamodel/common.adoc#qualifier-attributes[Qualifier].
There might be more than one qualifier per xref:spec-metamodel/common.adoc#qualifiable-attributes[Qualifiable].

There are different subtypes of SubmodelElements like properties, operations, lists, etc.
See  xref:spec-metamodel/submodel-elements.adoc[] for details.
A typical SubmodelElement is shown in the overview figure: a property is a data element that has a value of simple type like string, date, etc.
Every data element is a SubmodelElement (not visible in the figure but implicitly the case, since xref:spec-metamodel/submodel-elements.adoc#data-element-and-overview-of-data-element-types[DataElement]    is inheriting from xref:spec-metamodel/core.adoc#submodel-element-attributes[SubmodelElement]).
For details on properties, see  xref:spec-metamodel/submodel-elements.adoc#property-attributes[Property Attributes].

Every SubmodelElement needs a semantic definition (_semanticId_ in xref:spec-metamodel/common.adoc#has-semantics-attributes[HasSemantics]) to have a well-defined meaning.
The SubmodelElement might either refer directly to a corresponding semantic definition provided by an external reference (e.g. to an ECLASS or IEC CDD property definition), or it may indirectly reference a concept description (xref:spec-metamodel/concept-description.adoc[]).
...
Attributes for information like _preferredName_, _unit_ etc. are added to increase usability. For details see IDTA-01003-a.
Consistency w.r.t. the referenced SubmodelElement definitions should be ensured by corresponding tooling.
====

If a concept description is not just a copy or refinement of an external standard, the provider of the Asset Administration Shell using this concept description shall be aware that interoperability with other Asset Administration Shells cannot be ensured.
...
Data specification templates like the template for IEC 61360 property definitions (Part 3a, IDTA-01003-a) are explicitly predefined and recommended to be used by IDTA.
See  xref:data-specifications.adoc[] for details.
If proprietary templates are used, interoperability with other Asset Administration Shells cannot be ensured.

Besides SubmodelElements including properties and concept descriptions, other identifiable elements may also use additional templates (xref:spec-metamodel/common.adoc#has-data-specification-attributes[HasDataSpecification]).
