...
To get only the idShortPaths of a SubmodelElement hierarchy, the serialization format is specified in terms of an idShortPath notation to be returned in an JSON array.
...
In the first case, the SubmodelElement's xref:spec-metamodel/common.adoc#Referable[idShort] is separated by "." (dot) from top level down to child level.
...
*** SML: MySubSubmodelElementList3
**** SMC: MySubmodelElementCollection2InSML3
***** Property: "MySubTestValue3"
====
