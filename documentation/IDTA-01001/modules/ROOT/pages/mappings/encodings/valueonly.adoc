...
====
Note: 
xref:spec-metamodel/submodel-elements.adoc#Operation[Operation/inputVariables]  is renamed to "inputArguments" in the JSON Value-Only Schema.
====

This would be the specification as defined in the Metamodel for an operation with an input argument "arg1" of type "xs:int" and a return argument with name "arg2" of type "xsd:int":
...
The operation result in format "Normal" would look like this (here operation assumed to just add 1 to "arg1")
