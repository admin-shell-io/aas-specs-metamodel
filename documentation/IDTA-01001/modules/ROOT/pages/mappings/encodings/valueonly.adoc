...
====
Note: 
xref:spec-metamodel/submodel-elements.adoc#Operation[Operation/inputVariables] is renamed to "inputArguments" in the JSON Value-Only Schema etc.
====

This would be the specification as defined in the Metamodel for an operation with an input arguments "arg1" and "arg2" of type "xs:int" and a return argument with name "out1" of type "xsd:int":
...
The payload of the request body in format "Normal" for invoking an operation via API-Operation "InvokeOperation" would look like this for this operation:
...
The operation result in format "Normal" would look like this (here operation assumed to just add value of "arg2" to "arg1")
...
When invoking the same operation in Value-Only format the request body would look like this:
