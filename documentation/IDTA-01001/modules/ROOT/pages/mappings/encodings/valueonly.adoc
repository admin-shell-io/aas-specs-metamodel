...
xref:spec-metamodel/submodel-elements.adoc#Capability[Capabilities] and xref:spec-metamodel/submodel-elements.adoc#Operation[Operations] are excluded from the serialization scope since only data containing elements are in the focus.
They are consequently omitted in the serialization.

Abstract SubmodelElements are not contained in payload.

The following rules shall be adhered to when serializing a submodel, a SubmodelElement collection or a SubmodelElement list with the format "Value"footnote:[see SerializationModifier in Part 2 - IDTA-01002 - of the Specification of the Asset Administration Shell]:

* A Submodel or a SubmodelElement collection is serialized as an unnamed JSON object.
* A SubmodelElement list is serialized as an JSON array.
* A SubmodelElement is considered a leaf SubmodelElement if it does not contain other SubmodelElements.
A leaf SubmodelElement follows the rules for the different SubmodelElements considered in the serialization, as described below.
If it is not a leaf element, the serialization rules must be transitively followed until the value is a leaf SubmodelElement.
...
* For each SubmodelElement within the submodel, the SubmodelElement collection or SubmodelElement list:
...
The first is named "statements" ${Entity/statements} and contains an array of the serialized SubmodelElements according to their respective serialization mentioned in this clause.
...
The following rules shall be adhered to when serializing a single SubmodelElement with the format "Value":
...
SubmodelElements defined in the Submodel other than the ones mentioned above are not subject to the Value-Only serialization.

Optional elements (like for example globalAssetId for an Entity SubmodelElement) with no value shall be omitted in the serialization.

====
Note: Elements within a SubmodelElementList are neither optional or mandatory per se. 
So in Value-Only all elements within a SubmodelElementList shall be displayed even if they are empty (handled as if they are mandatory). 

Rationale: index of elements within a SubmodelElementList shall be identical in Normal and Value-Only format.
====

==== Data type to value mapping

The serialization of SubmodelElement values is described in the following table.
The left column "Data Type" shows the data types which can be used for SubmodelElement values.
...
The right column comprises related examples of the serialization of SubmodelElement values.
...
====
Note: Similar handling is required in case there are access rules disallowing access to specific SubmodelElements:
The protected elements shall not be serialized.
====

==== Examples Value-Only serialization for all SubmodelElement types 

In the following examples for Value-Only serializations for all SubmodelElement types are given.
...
For a _SubmodelElementList_ named "ListOfLists" with string SubmodelElementList as its value, the Value-Only payload is minimized to the following (empty lists are included):
...
----
{
    ...
    "value": "https://myDomain/SafetyInstructions.pdf"
}
...
=== Value-Only for Invoking of Operations

Although Operations are omitted in Value-Only there is one exception: invoking of operations.


====
Note: 
xref:spec-metamodel/submodel-elements.adoc#Operation[Operation/inputVariables] is renamed to "inputArguments" etc. in the JSON Value-Only Schema.

Same holds for the specification of the invoke operations in IDTA-01002 (InvokeOperationSync and InvokeOperationAsync). 
====

This would be the payload conformant to the metamodel specification as would be used in a Submodel Template Specification:
...
This would be the payload as defined in the API OpenAPI specification for the invocation of an operation (link:https://industrialdigitaltwin.io/aas-specifications/IDTA-01002/v3.2/specification/interfaces.html#InvokeOperationSync[InvokeOperationSync] 
with data type link:https://industrialdigitaltwin.io/aas-specifications/IDTA-01002/v3.1/specification/interfaces-payload.html#_operationrequest[OperationRequest] ) 
with idShort "add" in Format "Normal" with input arguments "arg1" and "arg2" of type "xs:int" and a return argument with name "out1" of type "xsd:int":
...
The operation result would look like this:
...
Arguments in an operation are handled like a SubmodelElementCollection, not like a SubmodelElementList because they may differ in type and semantics. 
The arguments in an operation are not ordered.

When invoking the same operation in Value-Only format the request body would therefore look like this:
