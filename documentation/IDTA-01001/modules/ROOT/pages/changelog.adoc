...
== Changes V3.3 vs. V3.2

Major Changes:

* NEW: add fingerprint to SubmodelElement File  (link:https://github.com/admin-shell-io/aas-specs-metamodel/issues/668[#668])
* NEW: add patch to AdministrativeInformation (link:https://github.com/admin-shell-io/aas-specs-metamodel/issues/684[#684])
* CHANGED AASd-005 to include AdministrativeInformation/patch
...
* INVALIDATED: experimental xref:spec-metamodel/common.adoc#Qualifier[Qualifier/kind] will not be supported in future (link:https://github.com/admin-shell-io/aas-specs-metamodel/issues/451[#451])
...
| {empty} | xref:spec-metamodel/common.adoc#Qualifier[Qualifier] a| attribute "kind" was experimental, will not be supported in future (invalidated)
| {empty} | xref:spec-metamodel/common.adoc#QualifierKind[QualifierKind] a| data type invalidated since xref:spec-metamodel/common.adoc#Qualifier[Qualifier/kind] invalidated 
| {empty} | xref:spec-metamodel/common.adoc#AdministrativeInformation[AdministrativeInformation] a| add new attribute patch to AdministrativeInformation 
...
[cols="5%,42%,48%",options="header",]
|===
h|Nc h|New Elements V3.3 vs V3.2 h|Comment
|{empty} | xref:spec-metamodel/submodel-elements.adoc#File[File/fingerprint]  a| new attribute with cardinality 0..*
|{empty} | xref:spec-metamodel/datatypes.adoc#FingerprintType[FingerprintType]  a| new datatype for xref:spec-metamodel/submodel-elements.adoc#File[File/fingerprint]
|{empty} | xref:spec-metamodel/common.adoc#AdministrativeInformation[AdministrativeInformation/patch]  a| new attribute with cardinality 0..*
|{empty} | xref:spec-metamodel/datatypes.adoc#PatchType[PatchType]  a| new datatype for xref:spec-metamodel/common.adoc#AdministrativeInformation[AdministrativeInformation/patch]
...
* CHANGED: description of AASd-002, mention that idShort consists of at least two characters (as specified in regEx) (https://github.com/admin-shell-io/aas-specs-metamodel/issues/641[#641])
...
* ADDED: Note to data type "Identifier" to use existing standards (https://github.com/admin-shell-io/aas-specs-metamodel/issues/658[#658])
* ADDED: Example and Note for Value-Only for SubmodelElementLists in SubmodelElementList: empty lists included (https://github.com/admin-shell-io/aas-specs-metamodel/issues/644[#644])
...
* REMOVED: Note 3 to _Operation_ since not possible due to Constraint AASd-134: even if the SubmodelElement as the value of an input and an output variable have the same idShort, this does not mean that they are identical or mapped to the same variable since OperationVariables are no referables. The same applies to two input variables or an input variable and an inoutputVariable a.s.o. (https://github.com/admin-shell-io/aas-specs-metamodel/issues/462[#462])
...
* ADDED: Note that AdministrativeInformation/templateId only applicable for Submodels so far
* REMOVED: Note for _Property/valueId_ that it is recommended to use externa references (was removed already in last release for most similar cases)
* REMOVED: Annex UML - since packages not used any longer remove explanation for UML packages
* CHANGES: editorial
...
| x |xref:spec-metamodel/submodel-elements.adoc#Range[Range] a| remove conditions "If the min value is missing, the value is assumed to be negative infinite." and "If the max value is missing, the value is assumed to be positive infinite." 
   ...
   Note 2: Range is a SubmodelElement in state "experimental", therefore non-backward-compatible changes are possible
...
add "[idShort] shall consist of at least two characters"

Constraint AASd-002: _idShort_ of __Referable__s shall consist of at least two characters and shall only feature letters, digits, hyphen ("-") and  underscore ("_"); starting mandatory with a letter, and not ending with a hyphen, i.e. ^[a-zA-Z][a-zA-Z0-9_-]*[a-zA-Z0-9_]+$.].

|{empty}  |AASd-108 |Update a|
use "SubmodelElement" instead of "submodel element"

|{empty}  |AASd-116 |Update a|
use \https://admin-shell.io/aas/3/x/AssetInformation/globalAssetId instead of \https://admin-shell.io/aas/3/1/AssetInformation/globalAssetId

Constraint AASd-116:
"globalAssetId" (case-insensitive) is a reserved key for SpecificAssetId/name with the semantics as defined in \https://admin-shell.io/aas/3/x/AssetInformation/globalAssetId, x being the minor version of the used specification.

|{empty}  |AASd-128 |Update a|
use "SubmodelElement" instead of "submodel element" 

|{empty}  |AASd-129 |Update a|
* add "Exception: the SubmodelElement is part of an OperationVariable." 
* use "SubmodelElement" instead of "submodel element" and "Submodel" instead of "submodel"

Constraint AASd-129: If any Qualifier/kind value of a SubmodelElement/qualifier (attribute qualifier inherited via Qualifiable) is equal to TemplateQualifier, the SubmodelElement shall be part of a Submodel template, 
i.e. a Submodel with Submodel/kind (attribute kind inherited via HasKind) value equal to Template. 
Exception: the SubmodelElement is part of an OperationVariable.

|(x)  |AASd-137 |New a|

Constraint AASd-137: 
For external references, i.e. xref:ROOT:spec-metamodel/referencing.adoc#Reference[Reference] the value of any xref:ROOT:spec-metamodel/referencing.adoc#Key[Key/type] contained in the Reference shall not be one of xref:ROOT:spec-metamodel/referencing.adoc#AasReferables[AasReferables].

|(x)  |AASd-138 |New a|

Constraint AASd-138: 
A SubmodelElementList within a Submodel of kind=Template or as part of an OperationVariable shall have exactly one element.
|===
