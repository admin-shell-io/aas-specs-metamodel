...
* NEW: new Constraint AASd-135 for References (https://github.com/admin-shell-io/aas-specs-metamodel/issues/592[#592])
* CHANGED: changed description of Constraint AASd-116, made more generic to avoid frequent updates
* ADDED: A minimum recursion depth of 32 for Container-Elements should be supported. Note: This means for certification a maximum of 32 recursion test cases should be tested.
...
| x |Range a| remove conditions "If the min value is missing, the value is assumed to be negative infinite." and "If the max value is missing, the value is assumed to be positive infinite." 
   ...
   Note 2: Range is an SubmodelElement in state "experimental", therefore non-backward-compatible changes are possible
...
Constraint AASd-002: _idShort_ of __Referable__s shall shall consist of at least two characters and shall only feature letters, digits, hyphen ("-") and  underscore ("_"); starting mandatory with a letter, and not ending with a hyphen, i.e. ^[a-zA-Z][a-zA-Z0-9_-]*[a-zA-Z0-9_]+$.].
