...
xref:annex/general.adoc#types-and-instances[Types and Instances] describes some general aspects of handling type and instance assets.

xref:annex/general.adoc#identification-of-elements[Identification of Elements] explains the very important aspects of identification in the context of the Asset Administration Shell.

xref:annex/general.adoc#matching-strategies[Matching Strategies] provides matching strategies for semantic identifiers and references.

xref:annex/general.adoc#submodel-instances-and-templates[Submodel Instances and Templates] explains the difference between submodel instances and templates.

xref:annex/general.adoc#events[Events] discusses aspects of event handling.
...
====
Note 4: for submodels representing the Bill of Material of a complex asset, the metamodel not only provides the possibility to define relationships (via the submodel element xref:spec-metamodel/submodel-elements.adoc#relationship-element-attributes[RelationshipElement], see above), 
it also explicitly supports the representation of another asset (via the submodel element xref:spec-metamodel/submodel-elements.adoc#entity-attributes[Entity]).
...
A so-called xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId] should be defined for this submodel as well as the submodel element.
...
* With exact matching, two semantic IDs need to represent two exact matching references (see xref:annex/general.adoc#matching-algorithm-for-references[Matching Algorithm for References]).
** Example: xref:ROOT:spec-metamodel/submodel-elements.adoc#Property[Property]  with xref:ROOT:spec-metamodel/common.adoc#Referable[idShort]  "ManufacturerName" + xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId]  value 0173-1#02-AAO677#002 and Property with idShort "Herstellername" + semanticId value 0173-1#02-AAO677#002 have exactly equal semantics if the xref:ROOT:spec-metamodel/referencing.adoc#Key[Key/type]s in both semanticIds are identical plus the other attributes of a xref:ROOT:spec-metamodel/referencing.adoc#Reference[Reference] like xref:ROOT:spec-metamodel/referencing.adoc#Reference[type] and xref:ROOT:spec-metamodel/referencing.adoc#Reference[referredSemanticId] as well. So if the Reference types differ or if the first key type of property "ManufacturerName" is xref:ROOT:spec-metamodel/referencing.adoc#ReferenceTypes[GlobalReference] and the first key type of the semanticId of property  "Herstellername" is xref:ROOT:spec-metamodel/referencing.adoc#ReferenceTypes[ConceptDescription] then the two semanticIds are no exact match. Or if the first reference contains a referredSemanticId value and the second does not the two semantic IDs are not exact matches.
...
* With value matching, the key values of the two semantic IDs being compared need to be string-identical, i.e. the two references need to value match (see xref:annex/general.adoc#matching-algorithm-for-references[Matching Algorithm for References]).
...
The provider of the Administration Shell can form in-house identifiers for the type and instance of the submodel in line with  xref:annex/general.adoc#identification-of-elements[Identification of Elements].
